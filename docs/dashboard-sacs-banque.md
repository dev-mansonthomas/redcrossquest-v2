...
Page Angular native permettant de consulter les sacs de banque (pièces et billets) par année.Fonctionnalité optionnelle — tous les UL n'utilisent pas les sacs de banque.
...
Les unités locales remettent l'argent dans des sacs de banque. Les sacs ne doivent pas dépasser un certain poids.Un sac contient soit des pièces, soit des billets (jamais les deux).Chaque tronc_queteur a 2 colonnes : `coins_money_bag_id` et `bills_money_bag_id`.Un même ID de sac peut être utilisé par plusieurs troncs (somme agrégée).
